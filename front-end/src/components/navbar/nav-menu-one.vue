...
<template>
    <div class="flex items-center gap-4 sm:gap-6">
        ...
        <button class="relative hdr_cart_btn" @click="cartList = !cartList">
            <span
                class="absolute w-[22px] h-[22px] bg-secondary top-[0px] -right-[11px] rounded-full flex items-center justify-center text-xs leading-none text-white">{{ cartCount
                }}</span>
        ...
        <div v-if="cartList"
            ...
            <div>
                <div class="hdr-cart-item" v-for="item in cart?.items" :key="item.id">
                    <div
                        class="flex gap-[15px] relative pb-[15px] mb-[15px] border-b border-bdr-clr dark:border-bdr-clr-drk group">
                        <router-link :to="`/product-details/${item.product_id || ''}`" class="block">
                            <img class="w-[70px] md:max-w-auto h-full" :src="item.thumbnail || w1" alt="cart" />
                        </router-link>
                        <div>
                            <div class="flex items-center gap-2">
                            <span class="text-[14px] leading-none">{{ item.product_title }}</span>
                            <span class="w-[6px] h-[6px] rounded-full bg-primary"></span>
                            <span class="text-[14px] leading-none">€{{ (item.unit_price).toFixed(2) }}</span>
                        </div>
                            ...
                            <IncDec :initial="item.quantity" @update="(qty) => cartStore.updateItem(item.id, qty)" />

                        </div>
                        <div @click="removeItem(item.id)"
                             class="wishList_item_close absolute top-0 right-0 w-6 h-6 flex items-center justify-center group hover:bg-primary">
                            <svg class="fill-current" width="10" height="10" viewBox="0 0 10 10" fill="none"
                    ...
                    <div class="pt-5 md:pt-[30px] mt-5 md:mt-[30px] border-t border-bdr-clr dark:border-bdr-clr-drk"></div>
                    </div>
                <div>
                    <h4 class="mb-5 md:mb-[30px] font-medium !leading-none text-lg md:text-xl text-right">
                        Subtotal : €{{ subtotal }}
                    </h4>
...
export default {
    ...
    computed: {
        ...
        subtotal() {
            return (
                this.cart?.items?.reduce((acc, item) => acc + (item.unit_price * item.quantity), 0) || 0
            ).toFixed(2);
        }
    },
    ...
    methods: {
        ...
        async removeItem(id) {
            await this.cartStore.removeItem(id)
            await this.cartStore.initCart() // ✅ reload updated cart into state
        },
        async updateQty(id, qty) {
            await this.cartStore.updateItem(id, qty)
        }
    },
    created() {
